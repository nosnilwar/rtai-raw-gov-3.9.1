...
static void fun(long t)
{
	...
	while(loops--) {
		...
		rt_printk("TESTE -> TASK %d with priority %d in loop %d \n", t, thread[t].priority,loops);

		rt_task_set_resume_end_times(-NTASKS*tick_period, -(t + 1)*tick_period);
	}
        rt_printk("TESTE -> TASK %d with priority %d  ENDS\n", t, thread[t].priority);
}


int __init init_edf_module(void)
{
	RTIME now;
	int i;
...
void __exit cleanup_edf_module(void)
{
	int i, cpuid;
	...
	printk("\n\nTESTE -> CPU USE SUMMARY\n");
	for (cpuid = 0; cpuid < NR_RT_CPUS; cpuid++) {
		printk("TESTE -> # %d -> %d\n", cpuid, cpu_used[cpuid]);
	}
	printk("TESTE -> END OF CPU USE SUMMARY\n\n");

	rt_printk(KERN_INFO "TESTE -> *************************** FIM *******************************\n");
}

module_init(init_edf_module);
module_exit(cleanup_edf_module);
MODULE_LICENSE("GPL");
